...
SCENE_XML = os.path.join(_ROOT, 'assets', 'mujoco', 'ur7e_scene.xml')
...
def render(episode_path, out_path, step, azimuth, elevation, distance):
    ...
    for count, i in enumerate(frame_indices):
        ...
        if writer is None:
            ...
            writer = cv2.VideoWriter(tmp_path, fourcc, 12, (composite.shape[1], composite.shape[0]))
        writer.write(composite)
        if count % 20 == 0:
            print(f'  frame {count}/{len(frame_indices)}')

    writer.release()
    # cv2.VideoWriter's mp4v (MPEG-4 Part 2) fails to open in most players/
    # browsers -- re-encode to H.264 the same way this repo's other render
    # scripts do, then drop the intermediate.
    subprocess.run(
